...
# def new_string(symbol, count = 3): # можем ввести символ и указать сколько раз его выводить. Если не укажем сколько, то по умолчанию он выведет его 3 раза
#     return symbol * count
# print(new_string('!', 5)) # если вместо символа введём число, то он умножит числа

def concatenatio(*params): #если перед агрументом поставить "*" , то
    res: str = "" # здесь указываем, что переменная является строкой. Если нужна цифра, то int = 0
    for item in params:
        res += item
    return res
